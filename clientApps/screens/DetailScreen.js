...
const DetailScreen = ({ route, navigation }) => {
  ...
  const youtubeApi = `https://api.themoviedb.org/3/${category === "1" ? "movie" : "tv"}/${movieId}/videos?api_key=464b6412840269fe91e87ba7d6958784&language=en-US`;
  const [dataYoutube, Youtubeloading] = useFetch(youtubeApi);

  const detailApi = `https://api.themoviedb.org/3/${category === "1" ? "movie" : "tv"}/${movieId}?api_key=464b6412840269fe91e87ba7d6958784&language=en-US`;
  const [dataDetail, Detailloading] = useFetch(detailApi);

  const creditApi = `https://api.themoviedb.org/3/${category === "1" ? "movie" : "tv"}/${movieId}/credits?api_key=464b6412840269fe91e87ba7d6958784&language=en-US`;
  const [dataCredit, Creditloading] = useFetch(creditApi);

  const similiarApi = `https://api.themoviedb.org/3/${category === "1" ? "movie" : "tv"}/${movieId}/similar?api_key=464b6412840269fe91e87ba7d6958784&language=en-US&page=1`;
          ...
          <View>
            ...
            <ScrollView>
              <View style={{ marginBottom: 50 }}>
                <Text style={{ fontSize: 30, fontWeight: "bold", color: "white", marginHorizontal: 20, marginTop: 10, marginBottom: 5 }}>{category === "1" ? dataDetail.title : dataDetail.name}</Text>
                ...
                <View style={{ marginHorizontal: 20, marginTop: 20, flexDirection: "row", height: 60 }}>
                  <View style={{ marginRight: 100 }}>
                    ...
                    <Text style={{ fontSize: 20, fontWeight: "bold", color: dataDetail.vote_average < 6 ? "red" : "green" }}>{dataDetail.vote_average * 10}</Text>
                  </View>
                  <View>
                    <Text style={{ fontSize: 20, fontWeight: "bold", color: "white" }}>{category === "1" ? "Release Date" : "First Air Date"}</Text>
                    <Text style={{ fontSize: 20, fontWeight: "bold", color: "grey" }}>{category === "1" ? dataDetail.release_date : dataDetail.first_air_date}</Text>
                  </View>
                </View>
                ...
                <View style={{ marginHorizontal: 20, marginTop: 20, marginBottom: 300 }}>
                  ...
                  {choose === 1 && <FlatList horizontal={true} data={credit} renderItem={({ item, index }) => <CreditCast list={item} />} keyExtractor={(key, index) => index.toString()} />}
                  {choose === 2 && (
                    <View>
                      <FlatList
                        horizontal={true}
